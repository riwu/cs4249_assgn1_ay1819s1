...
function nextTrial() {
  if (currentTrial <= numTrials) {
    ...
    document.getElementById('trialNumber').innerHTML =
      String(currentTrial) + '/' + String(numTrials);
    ...
    if (menuType === 'Marking') {
      ...
      if (menuDepth == 1) {
        menu = MarkingMenu(
          markingMenuL1,
          document.getElementById('marking-menu-container')
        );
      } else if (menuDepth == 2) {
        menu = MarkingMenu(
          markingMenuL2,
      ...
      markingMenuSubscription = menu.subscribe(selection =>
...
function initializeMarkingMenu() {
  ...
  if (markingMenuContainer == null) {
    interactionContainer.innerHTML +=
      '<div id="marking-menu-container" style="height:100%;width:100%" onmousedown="markingMenuOnMouseDown()" oncontextmenu="preventRightClick(event)"></div>';
  }
}
...
function initializeRadialMenu() {
  ...
  if (radialMenuContainer == null) {
    interactionContainer.innerHTML +=
      '<div id="radial-menu-container" style="height:100%;width:100%" oncontextmenu="toggleRadialMenu(event)"></div>';
  }
}
...
function createRadialMenu(radialMenuL) {
  ...
  radialMenuSvg = d3
    .select('#radial-menu-container')
    .append('svg')
    .attr('width', w)
    .attr('height', h)
    .attr('id', 'radial-menu-svg');
